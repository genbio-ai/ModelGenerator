...
def align_adata(adata: ad.AnnData) -> Tuple[ad.AnnData, np.ndarray]:
    """Aligns the input AnnData object to the GB.Cell gene set.
    ...
    Returns:
        Tuple[ad.AnnData, np.ndarray]: A tuple containing the aligned AnnData object and an attention mask.
            The aligned AnnData object has the same genes as GB.Cell, and the attention mask indicates which genes
            are present in the GB.Cell pretraining set.
    """
    print('###########  Aligning data to GB.Cell  ###########')
    print(f'GB.Cell was pretrained on a fixed set of {len(MODEL_GENES)} genes.')
    print('Aligning your data to the GB.Cell gene set...')
    missing_genes = np.setdiff1d(MODEL_GENES, adata.var.index)
    new_missing_genes = np.setdiff1d(adata.var.index, MODEL_GENES)
    print(f'{len(new_missing_genes)} in your data that cannot be used by GB.Cell. Removing these.')
    print(new_missing_genes)
    print(f'{len(missing_genes)} genes in the GB.Cell pretraining set missing in your data.')
    print('Setting unknown genes to 0.')
    # Todo: GB.Cell is trained with -1 as mask token, but read depth features don't work with -1 sum for read counts
    print(missing_genes)
    ...
    print('Reordering genes to match GB.Cell gene ordering')
