...
finetune bash -c "mgen fit --config experiments/GB.Cell/cell_type_classification.yaml"
```
...
-v /home/user/.cache/huggingface:/root/.cache/huggingface \
-v "/home/user/ModelGenerator/logs:/workspace/logs" \
finetune bash -c "mgen test --config experiments/GB.Cell/cell_type_classification.yaml --ckpt_path /workspace/lightning_logs/version_X/checkpoints/my.ckpt"
```
...
Simply replace the `config` argument of `mgen fit` with  `experiments/GB.Cell/transcriptomic_clock.yaml`.
