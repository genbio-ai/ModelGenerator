...
GB.ModelGenerator implements the procedure proposed by [Tomaz da Silva et al.](https://www.biorxiv.org/content/10.1101/2024.07.27.605418v1)
We use this to mine functional genomic elements in the [GB.DNA](https://doi.org/10.1101/2024.12.01.625444) paper with the [GB.DNA-7B](https://huggingface.co/genbio-ai/GB.DNA-7B) and [GB.DNA-300M](https://huggingface.co/genbio-ai/GB.DNA-300M) models.
This task uses the pre-trained models directly, and does not require finetuning.

To reproduce the dependency mapping results from the GB.DNA paper, run the following from the ModelGenerator root directory:
```
# Inference
mgen predict --config experiments/GB.DNA/dependency_mapping/config.yaml

# Plotting
python experiments/GB.DNA/dependency_mapping/plot_dependency_maps.py \
    -i depmap_predictions \
    -o depmap_plots \
    -v experiments/GB.DNA/dependency_mapping/DNA.txt \
...
data:
  ...
  init_args:
    ...
    test_split_files:
      - <my_sequences.tsv>
    vocab_file: <vocab>.txt  # E.g. experiments/GB.DNA/dependency_mapping/DNA.txt
trainer:
  callbacks:
