...
The primary objective of the DMS Benchmark is to model protein fitness landscapes - the complex relationships between genetic mutations and their impacts on protein fitness and functionality. We have fine-tuned [GB.Protein-RAG-16B](https://huggingface.co/genbio-ai/GB.Protein-RAG-16B) models using this benchmark. The implementation in GB.ModelGenerator employs LoRA fine-tuning with a 5-fold cross-validation scheme, following the random split strategy described in the original [ProteinGym paper](https://www.biorxiv.org/content/10.1101/2023.12.07.570727v1). For efficient training, the system utilizes Distributed Data Parallel (DDP).

Note:​​ All subsequent scripts should be executed within the `ModelGenerator/experiments/GB.Protein-RAG/DMS_RAG` directory.
...
# GB.Protein-RAG-3B (without structural input support)
./run_dms.sh --nodes node1,node2 --mask-str --backbone modelgenerator.backbones.gb_protein_rag_3b Q2N0S5_9HIV1_Haddox_2018
```
...
# GB.Protein-RAG-3B
./run_dms_eval.sh --node node_name --mask-str --backbone modelgenerator.backbones.gb_protein_rag_3b Q2N0S5_9HIV1_Haddox_2018
```
