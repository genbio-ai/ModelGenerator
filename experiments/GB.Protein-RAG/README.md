...
GB.Protein-RAG-16B is a multimodal protein language model that integrates Multiple Sequence Alignment (MSA) and structural data, building upon the [GB.Protein-16B](https://huggingface.co/genbio-ai/GB.Protein-16B) foundation. The training process comprises three main stages:
...
For more details, please refer to the [GB.Protein-RAG](https://huggingface.co/genbio-ai/GB.Protein-RAG-16B) model card, and our manuscipts
...
The primary objective of the DMS Benchmark is to model protein fitness landscapes - the complex relationships between genetic mutations and their impacts on protein fitness and functionality. We have fine-tuned [GB.Protein-RAG-16B](https://huggingface.co/genbio-ai/GB.Protein-RAG-16B) models using this benchmark, and uploaded a new version with MSAs and embeddings from GB.StructureTokenizer to [ProteinGYM-DMS-RAG](https://huggingface.co/datasets/genbio-ai/ProteinGYM-DMS-RAG) on HuggingFace.

The implementation in GB.ModelGenerator employs LoRA fine-tuning with a 5-fold cross-validation scheme, following the random split strategy described in the original [ProteinGym paper](https://www.biorxiv.org/content/10.1101/2023.12.07.570727v1). For efficient training, the system utilizes Distributed Data Parallel (DDP).
...
mgen fit \
    ...
    --model.init_args.backbone.class_path "gb_protein_rag_16b" \
