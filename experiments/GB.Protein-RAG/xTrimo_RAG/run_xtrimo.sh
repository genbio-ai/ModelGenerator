...
# RUN_NAME=fold_GB.Protein.RAGPLM.w.structure
...
if [[ "$BACKBONE" != "null" ]]; then
    if [[ "$BACKBONE" != "modelgenerator.backbones.gb_protein_rag_3b" ]] && [[ "$BACKBONE" != "modelgenerator.backbones.gb_protein_rag_16b" ]] && [[ "$BACKBONE" != "modelgenerator.backbones.aido_protein_3b" ]];
    then
        echo -e "\x1b[0;31;49m Error: --backbone only support modelgenerator.backbones.gb_protein_rag_3b, modelgenerator.backbones.gb_protein_rag_16b and modelgenerator.backbones.aido_protein_3b \x1b[0m"
        exit 1;
    fi

    if [[ "$BACKBONE" == "modelgenerator.backbones.gb_protein_rag_3b" ]] && [[ "$STR_EMBEDDING_IN" != "0" ]]; then
        echo -e "--mask-str must be specified when use modelgenerator.backbones.gb_protein_rag_3b as backbone"
        exit 1;
    fi
