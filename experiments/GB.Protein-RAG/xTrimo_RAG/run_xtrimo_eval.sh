...
if [[ "$BACKBONE" != "null" ]]; then
    if [[ "$BACKBONE" != "modelgenerator.backbones.gb_protein_rag_3b" ]] && [[ "$BACKBONE" != "modelgenerator.backbones.gb_protein_rag_16b" ]];
    then
        echo -e "\x1b[0;31;49m Error: --backbone only support modelgenerator.backbones.gb_protein_rag_3b or modelgenerator.backbones.gb_protein_rag_16b \x1b[0m"
        exit 1;
    fi
