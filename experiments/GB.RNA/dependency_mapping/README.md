...
We use this to assess structural features learned during pretraining in the [GB.RNA](https://www.biorxiv.org/content/10.1101/2024.11.28.625345v1) paper with the [GB.RNA-1.6B](https://huggingface.co/genbio-ai/GB.RNA-1.6B) model.
This task uses the pre-trained models directly, and does not require finetuning.

To reproduce the dependency mapping results from the GB.RNA paper, run the following from the ModelGenerator root directory:
```
# Inference
mgen predict --config experiments/GB.RNA/dependency_mapping/config.yaml

# Plotting
python experiments/GB.RNA/dependency_mapping/plot_dependency_maps.py \
    -i depmap_predictions \
    -o depmap_plots \
    -v experiments/GB.RNA/dependency_mapping/DNA.txt \
...
data:
  ...
  init_args:
    ...
    test_split_files:
      - <my_sequences.tsv>
    vocab_file: <vocab>.txt  # E.g. experiments/GB.RNA/dependency_mapping/DNA.txt
trainer:
  callbacks:
