...
if [ $MODE == "train" ]; then
  for FOLD in {0..4}
  do
    RUN_NAME=pa_${ORGANISM}_gb_rna_1b600m_cds_fold${FOLD}
    CKPT_SAVE_DIR=logs/${PROJECT}/${RUN_NAME}
    mgen fit --config experiments/GB.RNA/configs/protein_abundance.yaml \
  ...
  do
    ...
    echo ">>> Fold ${FOLD}"
    mgen test --config experiments/GB.RNA/configs/protein_abundance.yaml \
      --data.config_name protein_abundance_${ORGANISM} \
      --data.cv_test_fold_id $FOLD \
