...
As with proteins, structure determines RNA function. RNA secondary structure, formed by base pairing, is more stable and accessible than its tertiary form within cells. Accurate prediction of RNA secondary structure is essential for tasks such as higher-order structure prediction and function prediction. As discussed in our paper [GB.RNA](https://doi.org/10.1101/2024.11.28.625345), we finetune the [GB.RNA-1.6B](https://huggingface.co/genbio-ai/GB.RNA-1.6B) model on the training splits of the following two datasets:
...
| **RNA family**     | **GB.RNA (ours)** | **RNAstructure** | **CONTRAfold** | **RiNALMo** | **RNA-FM** | **MXfold2** | **UFold** |
|--------------------|---------------------|------------------|----------------|-------------|------------|-------------|-----------|
...
#### To finetune [GB.RNA-1.6B](https://huggingface.co/genbio-ai/GB.RNA-1.6B) on RNA SS:
...
```
2. Alternatively, to finetune on Archive-II datasets (for the inter-family generalization experiment discussed in the paper [GB.RNA](https://doi.org/10.1101/2024.11.28.625345)), run the following command:
```
...
```
Here, the [GB.RNA-1.6B](https://huggingface.co/genbio-ai/GB.RNA-1.6B) model will be finetuned using **all other splits except archiveII_5s**.

#### To test a finetuned checkpoint on RNA SS:
- Finetune [GB.RNA-1.6B](https://huggingface.co/genbio-ai/GB.RNA-1.6B) as discussed above, **or** download the `model.ckpt` checkpoint from [here](https://huggingface.co/genbio-ai/GB.RNA-1.6B-inv-fold).
