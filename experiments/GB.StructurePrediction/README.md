# [GB](https://github.com/genbio-ai/aido).StructurePrediction
...
GB.StructurePrediction is an AlphaFold3-like full-atom structure prediction model,
...
GB.StructurePrediction comes with a CLI tool `genbio-gbsp` to download model checkpoint and prepare the necessary data for inference.
...
cd ModelGenerator/experiments/GB.StructurePrediction
...
genbio-gbsp util download-model -o my_model_dir --model-version v0.1.2
genbio-gbsp util download-ccd -o my_ccd_dir
...
genbio-gbsp util retrieve-msa -i examples/example.fasta -o my_msa_dir -c configs/mmseqs.yaml --cpus-per-task 8 --no-tasks 1 --shuffle-file-list
...
You can download model checkpoint and CCD component files from [GenBio Huggingface](https://huggingface.co/genbio-ai/GB.StructurePrediction/tree/main) or use the packaged CLI.

```bash
genbio-gbsp util download-model -o <model_download_dir> --model-version v0.1.2 # The only available version for now
genbio-gbsp util download-ccd -o <ccd_download_dir>
```
...
```bash
genbio-gbsp util retrieve-msa -i <input_fasta_path> -o <msa_output_dir> -c configs/mmseqs.yaml --cpus-per-task <num_cpus> --no-tasks <num_tasks> --shuffle-file-list
```
...
# Run the container from the root of GB.StructurePrediction
...
- `checkpoint_path`: the model checkpoint path, you can download it in [Huggingface](https://huggingface.co/genbio-ai/GB.StructurePrediction/tree/main)
- `ccd_components_file`: ccd file. you can download it in [Huggingface](https://huggingface.co/genbio-ai/GB.StructurePrediction/tree/main)
- `ccd_components_rdkit_mol_file`: ccd file. you can download it in [Huggingface](https://huggingface.co/genbio-ai/GB.StructurePrediction/tree/main)
...
<img src="assets/img/vis.png" alt="GB.StructurePrediction outputs"/>

## Local MSA Database

GB.StructurePrediction uses the MMseqs2 API to retrieve MSA data.
This is a great option for one-off predictions, but it can be slow and unreliable for benchmarking or frequent use.

If you plan to use GB.StructurePrediction regularly, we recommend downloading and creating your MSA databses locally.
...
```bash
genbio-gbsp util download-db -o <output_dir> -n colabfold_envdb -n uniref30
```

This effectively runs bash scripts in the [scripts](src/genbio/gbsp/scripts) directory, downloading and using `tar` to unpack the databases. You can also download the databases manually.

```bash
bash src/genbio/gbsp/scripts/download_colabfold_envdb.sh <output_dir>
bash src/genbio/gbsp/scripts/download_uniref30.sh <output_dir>
```
...
<img src="assets/img/hln.png" width="500" lt="GB Antibody/Nanobody Benchmark Results" width="500" />
<img src="assets/img/ana.png" width="500" alt="GB Antibody/Nanobody-Antigen by DcokQ" width="500" />
...
```bash
eval "$(_GENBIO_GBSP_COMPLETE=bash_source genbio-gbsp)"
```
...
```bash
eval "$(_GENBIO_GBSP_COMPLETE=zsh_source genbio-gbsp)"
```

### Fish

Add this to `~/.config/fish/completions/genbio-gbsp.fish`:

```fish
_GENBIO_GBSP_COMPLETE=fish_source genbio-gbsp | source
```
...
```bash
genbio-gbsp completion --shell fish > ~/.config/fish/completions/genbio-gbsp.fish
```
...
Please cite GB.StructurePrediction using the following BibTex code:

```bibtex
@inproceedings{aido_structurepediction,
 title = {GB StructurePrediction},
 url = {https://huggingface.co/genbio-ai/GB.StructurePrediction},
