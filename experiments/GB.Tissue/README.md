# Finetuning GB.Tissue for spatial single cell downstream tasks
...
We fully finetune GB.Tissue for niche label type prediction.


#### Finetuning script
```shell
CUDA_VISIBLE_DEVICES=7 nohup mgen fit --config experiments/GB.Tissue/niche_type_classfification.yaml > logs/nohup/GB.Tissue.niche_type_classfification.yaml.log 2>&1 &
```
...
```shell
CUDA_VISIBLE_DEVICES=6 nohup mgen test --config experiments/GB.Tissue/niche_type_classfification.yaml \
  --ckpt_path ckpt_path \
...
The config file is like `experiments/GB.Tissue/cell_density_regression.yaml`, all the fintuning running and evaluation are similar as classification task.
...
```shell
CUDA_VISIBLE_DEVICES=3 nohup mgen predict --config experiments/GB.Tissue/emb.xenium.yaml > logs/nohup/GB.Tissue.emb.xenium.log 2>&1 &
```
