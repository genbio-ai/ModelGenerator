...
class BertSelfFlashAttention(Bert2SelfAttention):
    ...
    def forward(
        ...
        # Use FlashAttention if available (CUDA only), otherwise fallback to PyTorch SDPA
        if is_flash_attn_2_available() and my_query_states.is_cuda:
            ...
            # Reshape back to (bsz, nh, sq, hd) for SDPA
            q = my_query_states.reshape(bz, sq, nh, hd).transpose(1, 2)  # (bsz, nh, sq, hd)
            k = my_key_states.reshape(bz, sq, nh, hd).transpose(1, 2)
            v = my_value_states.reshape(bz, sq, nh, hd).transpose(1, 2)
