...
Standalone GB.Cell model repo using HuggingFace handles.
...
MODEL_NAME = "genbio-ai/GB.Cell-3M"  # Or "genbio-ai/GB.Cell-100M"
...
```python
# CONFIGURATION - Set these variables
MODEL_NAME = "genbio-ai/GB.Cell-3M"   # HuggingFace model handle
...
GB.Cell was pre-trained on a fixed set of 19,264 genes using a read depth-aware objective function.
All inputs should be processed using the `gb_cell.utils.gene_alignment` and `gb_cell.utils.preprocessing` tools.

1. Gene alignment 
  1. Removes genes in your data that aren't in GB.Cell's gene set
  2. Adds zero-filled entries for genes in GB.Cell's set that are missing from your data
  3. Reorders genes to match GB.Cell's expected order
...
- `genbio-ai/GB.Cell-3M`
- `genbio-ai/GB.Cell-10M`
- `genbio-ai/GB.Cell-100M`

Check the [GB.Cell HuggingFace page](https://huggingface.co/genbio-ai) for the latest models.
